...
struct Pos2D {
    ...
    bool operator!=(const Pos2D &rhs) const { return !operator==(rhs); }
};
...
struct RTTT {
    ...
    struct TTT {
        ...
        TTT(std::vector<std::vector<RTTT::TTT>> &top, Pos2D pos) : TTT(top.size(), top[0].size(), top[0][0].win_condition) {
            for (unsigned i = 0; i < top.size(); ++i) {
                for (unsigned j = 0; j < top[i].size(); ++j) {
                    ...
                    if (top[i][j].gameEnded()) --empty_cells;
                }
            }
        ...
        [[nodiscard]] uint8_t updateGameState(Pos2D pos) {
            ...
            for (unsigned x = 0; x < width; ++x) {
                ...
                if (team != 0 && times >= win_condition - 1) { return team; }
                ++times;
            }
            // vertically
            team = times = 0;
    ...
    Pos2D previous_pos, last_pos = Pos2D(0, 0);
    struct Layer {
    ...
    enum MoveErrorCode : uint8_t { OK = 0, INVALID_POS, INVALID_LAST_POS };
    MoveErrorCode move(Pos2D pos) {
        if (isLastPosInvalid()) { return INVALID_LAST_POS; }
        if (pos.x >= width || pos.y >= height || current_top[last_pos.x][last_pos.y].field[pos.x][pos.y] != 0) { return INVALID_POS; }
        auto &top = current_top[last_pos.x][last_pos.y];
        top.move(pos, player_to_move);
        player_to_move = (player_to_move + 1) % (players_n + 1);
        player_to_move += (player_to_move == 0);
        if (firstGame() ? top.gameEnded() : TTT(current_top, last_pos).gameEnded()) {
            ...
            if (firstGame()) {
                ...
                new_top[last_pos.x][last_pos.y] = TTT(current_top, last_pos);
                layers.push_back({std::move(current_top), last_pos});
            }
    ...
    bool isLastPosInvalid() { return last_pos.x >= current_top.size() || last_pos.y >= current_top[0].size() || current_top[last_pos.x][last_pos.y].gameEnded(); }
};
